...
public function getStatusInfo() {
    $INFO = array (
        ...
        "codes" => $this->main->dql("
            SELECT g.\"group\", p.module, MAX(p.version) AS version
            FROM phas_phas p
            LEFT JOIN phas_groups g ON p.group_id = g.id
            GROUP BY g.\"group\", p.module"),
        "groups" => $this->main->dql("SELECT id, \"group\" FROM phas_groups")
    );
